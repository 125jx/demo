...
// import windJson from '/public/new-wind-global1.json';  // 风场数据
  ...
  "features": [
    {
      ...
      "geometry": {
        "type": "LineString",
        "coordinates": [
          [112.1, 12.0], [111.9, 12.9], [111.6, 13.6], [111.2, 14.5],
          [110.9, 15.2], [110.8, 15.4], [110.6, 15.6], [110.5, 15.8],
          [110.5, 16.1], [110.6, 16.6], [111.1, 17.3], [111.3, 18.0],
          [111.4, 18.6], [111.3, 19.3], [111.1, 19.7], [111.0, 20.2],
          [110.8, 20.5], [110.5, 20.6], [110.2, 20.5], [110.2, 20.2],
          [110.3, 20.0], [110.5, 19.9], [110.8, 19.9], [111.0, 20.0],
          [111.1, 20.2], [111.2, 20.5], [111.3, 20.7], [111.4, 20.9],
          [111.5, 21.1], [111.7, 21.3], [111.8, 21.5], [111.9, 21.8],
          [112.0, 22.2], [112.1, 22.5], [112.2, 22.8], [112.3, 23.0],
          [112.4, 23.2], [112.5, 23.4], [112.7, 23.6], [112.9, 23.7],
          [113.2, 23.7], [113.4, 23.6], [113.6, 23.5], [113.7, 23.3],
          [113.8, 23.1], [114.0, 22.9], [114.2, 22.7], [114.5, 22.5],
          [114.9, 22.4], [116.3, 22.5]
        ]
      },
      "properties": {
        "id": "0005_2018",
        "start_time": "0005_2018060212",
        "end_time": "0005_2018061000",
        "max_wind_speed": 23,
        "min_pressure": 990
      }
    }
...
const initializeJsonLayer = () => {
  ...
  const animateTyphoon = () => {
    if (currentIndex < typhoonGeoJson.features[0].geometry.coordinates.length) {
      const currentCoordinates = typhoonGeoJson.features[0].geometry.coordinates.slice(0, currentIndex + 1);
      ...
      const currentEye = typhoonGeoJson.features[0].geometry.coordinates[currentIndex];
...
if (geoJsonLayer) {
  map.value.removeLayer(geoJsonLayer);  // 移除热力图
  geoJsonLayer = null;  // 清空热力图图层变量
}
};
